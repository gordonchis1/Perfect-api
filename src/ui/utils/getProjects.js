...
export async function getProjects() {
    try {
        ...
        const path = await window.path.join(paths.projectFilePath);
        const projectsData = await window.fs.readTextFile(path);
...
export async function getProjectById(id) {
    ...
    try {
        ...
        const data = await window.fs.readTextFile(path);
